...
func NewSeqAdapter(bc *core.BlockChain, client *ethclient.Client) (*SeqAdapter, error) {
...
func (s *SeqAdapter) GetTxSequencer(tx *types.Transaction, expectIndex uint64) (common.Address, error) {
	...
	if tx != nil {
		seqOper, data := s.IsSeqSetContractCall(tx)
		if seqOper {
			recommitEpoch := s.ParseUpdateSeqData(data)
			if recommitEpoch != nil {
				// respan
				log.Info("get tx seqeuencer respan", "respan-start", recommitEpoch.StartBlock.Uint64(), "respan-end", recommitEpoch.EndBlock.Uint64(), "respan-signer", recommitEpoch.NewSigner.String())
				// cache the respan start block
				s.cachedSeqEpoch.RespanArr = s.insertRespanNumber(s.cachedSeqEpoch.RespanArr, recommitEpoch.StartBlock)
				// return the respan sequencer, it will mine. it can success or fail
				return recommitEpoch.NewSigner, nil
			}
		}
	}
	...
	if tx != nil && err != nil && err.Error() == "get sequencer incorrect epoch number" {
		mpcAddress, err2 := s.contract.MpcAddress(nil)
		if err2 != nil {
		...
		if tx.To() == nil || *tx.To() != mpcAddress || tx.Value() == nil || tx.Value().Cmp(minValue) <= 0 {
		...
		rawBalance, err2 := s.client.BalanceAt(bgCtx, mpcAddress, nil)
		if err2 != nil {
